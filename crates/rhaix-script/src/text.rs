...
pub fn cut(text: &str, limit: usize) -> String {
    ...
    let trimmed = match head.rfind(char::is_whitespace) {
        ...
        // `rfind` дає БАЙТОВИЙ індекс, а `limit` — у символах: для кирилиці
        // (2 байти на літеру) порівнювати їх напряму означало б дозволити
        // відрізати до трьох чвертей рядка. Тому рахуємо символи.
        Some(index) if head[..index].chars().count() >= limit / 2 => &head[..index],
        _ => head.trim_end(),
    };
...
mod tests {
    ...
    #[test]
    fn cut_does_not_throw_away_most_of_a_cyrillic_line() {
        // Пробіл на 3-му символі з 20. У байтах це 6-та позиція — стара
        // перевірка `6 >= 10` не спрацьовувала б тут, але на 6-му символі
        // (12-й байт) спрацьовувала: різала 20 символів до 6.
        let text = "абв гдеєжзиіїйклмнопрстуф";
        assert_eq!(cut(text, 20), "абв гдеєжзиіїйклмноп…");
        // А тут стара версія віддавала лише "абвгде…".
        let text = "абвгде жзиіїйклмнопрстуфхцчшщ";
        assert_eq!(cut(text, 20), "абвгде жзиіїйклмнопр…");
    }
