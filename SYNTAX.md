...
`markdown()` є в `rhaix dev` і `rhaix serve` завжди. `rhaix build` вмикає його
(feature `markdown`), коли бачить виклик `markdown(` у `.rhx` чи `.rhai`
проєкту: застосунок без нього не тягне парсер markdown у бінарник.
