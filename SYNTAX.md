...
| Спецтег | Що вставляє |
|---|---|
| `<rhaix:head/>` | `public/**.css`, стилі компонентів, `page.head`, **і скрипти**: htmx, ядро `rhaix.js`, UI `ui.js` (7.7), `public/**.js` з `defer` |
| `<rhaix:scripts/>` | лише підняті `<script>` компонентів |
| `<rhaix:raw>…</rhaix:raw>` | блок без обробки |

Скрипти — у `<head>`, а не в кінці `<body>`, навмисно: htmx свопить лише
`<body>`, і скрипт звідти на boosted-переході виконався б знову (до 1.2.5 так і
було — кожен такий перехід додавав ще один слухач, і тости множились). Скрипти
проєкту мають `defer`: виконуються один раз, коли `<body>` уже є. Layout без
`<rhaix:head/>` теж працює — тоді все це йде в `<rhaix:scripts/>`, як раніше.
...
**CORS — `[api] cors`.** Якщо API кличе фронтенд з іншого домену:
...
| `state` | усюди | процесне сховище: `state.get/set/has/remove`; ліміт спроб: `state.allow/retry_after/reset` (7.2) |
...
**Обмеження спроб — `state.allow`.** Перебір паролів, спам формою, надто
...
**`req.ip`** — адреса клієнта. За власним зворотним проксі (nginx, Caddy)
...
форму. До 1.5.0 зайвий день тихо переносився на наступний місяць, а зіпсований
час ставав нулем.
...
**Відповіді з кодом, відмінним від 2xx, теж свопляться.** htmx за
...
**Валідація під полями — компонент, не директива.** Фреймворк навмисно не
...
**Тости.** `hx.toast(message, type)` (7.5) на сервері шле подію `showToast`
...
**Тост перед редіректом не губиться (flash).** `hx.toast("Вітаємо");
...
**`<dialog>` відкривається як справжній модал сам.** Досить писати `<dialog>`
