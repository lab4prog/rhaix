...
Хешування паролів свідомо не входить у v1: правильний `hash_password` — це
Argon2 з підбором параметрів, і видавати за нього SHA-256 нечесно. Це частина
M12 разом із рештою автентифікації.
...
- `<style>` у v1 **глобальний**: піднімається в `<rhaix:head/>`, дедуплікується за
  хешем вмісту. Скоупинг (`data-rhx-<hash8>` + переписування селекторів) вимагає
  повноцінного CSS-парсера і винесений у **[v1.1]**; атрибут `<style scoped>`
  зарезервовано під нього. Ізоляція поки — конвенцією імен класів.
