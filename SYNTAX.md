...
**Обробники подій.** `onclick={expr}`, `@attr={#{"onclick": …}}` і ключі `on*`
у `{...spread}` — відхиляються: перші дві як помилка компіляції, ключі зі spread
відкидаються в рантаймі з попередженням. Статичний `onclick="alert(1)"`,
написаний прямо в розмітці, працює як завжди.
...
Усередині доступний об'єкт `loop`:

| Поле | Значення |
|---|---|
| `loop.index` | 0-based індекс |
| `loop.number` | 1-based |
| `loop.first` / `loop.last` | `bool` |
| `loop.count` | довжина колекції |
