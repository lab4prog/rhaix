...
Наслідок: **layout на htmx-навігації взагалі не виконується**. Усе, що в
...
### 7.7 Клієнт: помилки все-таки доїжджають, тости й модалки — свої
...
діагностикою. Ядро перевизначає це одним рядком у `rhaix.js`, тому
`validate()` на сервері не лишається невидимим на екрані — жодних дій у
проєкті не потрібно.
...
**Тости малює `window.__rhaix.toast(message, type)`.** `hx.toast(...)` (7.5)
лишається як є — сервер лише шле подію `showToast` заголовком `HX-Trigger`.
Малюнок винесено в окрему, перевизначувану функцію:

```js
// public/app.js — підключається ПІСЛЯ rhaix.js, тож можна просто перезаписати:
window.__rhaix.toast = (message, type) => myToastLib.show(message, type);
```

Перевизначення читається під час самої події, а не запам'ятовується наперед,
тож працює незалежно від того, коли саме `app.js` це зробив.

**`<dialog>` відкривається як справжній модал сам.** Досить писати `<dialog>`
замість `<div class="modal">` — ядро саме викликає `showModal()` для кожного
