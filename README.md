...
- **`api/` → JSON**: `api/orders.rhx` → `/api/orders`; повернена мапа стає JSON
  сама, layout не додається, помилки й 404 теж машинні. Сесії там немає навмисно —
  тому CSRF не потрібен, а чужий сайт не може діяти від імені користувача;
...
- **тости й модалки — окремо від ядра**: `hx.toast(...)` на сервері, а малює
  ...
  модальним вікном. Ядро (`rhaix.js`) — лише протокол із htmx;
